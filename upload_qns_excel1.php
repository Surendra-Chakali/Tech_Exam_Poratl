...
foreach ($obj->getWorksheetIterator() as $sheet){
	$highestrow = $sheet->getHighestRow();
		for($i = 1;$i<=$highestrow;$i++){
            ...
            $opt1 = $sheet->getCellByColumnAndRow(2,$i)->getValue();
            $opt2 = $sheet->getCellByColumnAndRow(3,$i)->getValue();
            $opt3 = $sheet->getCellByColumnAndRow(4,$i)->getValue();
            $opt4 = $sheet->getCellByColumnAndRow(5,$i)->getValue();
            $ans = $sheet->getCellByColumnAndRow(6,$i)->getValue();
	$sql1 = mysqli_query($conn,"select * from questions where subjectname = '$subname' and sno = $i");
	$result = array();
...
if($i == $sn && $subjectname == $subname){
	mysqli_query($conn, "update questions set quation = '$qns', opt1 = '$opt1', opt2 = '$opt2', opt3 = '$opt3', opt4 = '$opt4', answer = '$ans', subjectname = '$subname', sno = $sno, year = '$year', departname = '$department_name' where subjectname = '$subname' && sno = $i");}
	else{
		mysqli_query($conn,"insert into questions values('$qns', '$opt1', '$opt2', '$opt3', '$opt4', '$ans',$subname','$sno', '$year', '$department_name')");}}
