...
include("db_connection.php");
      ...
      $result = mysqli_query($conn,"select * from result1 where rollnumber = '$rollnum' ");   
        if($result -> num_rows >0)
        { 
          echo "<table class='table table-hover text-center mt-4'><thead class=''><tr><th style='color:red;'>Subject</th><th style='color:green;'>Question</th><th style='color:blue;'>Answer</th></tr></thead>";
        while($data = mysqli_fetch_array($result))
        {
          $subname = $data['subject'];
          $qns = $data['question'];
          $ans = $data['answer'];
          echo "<tbody><tr><td>".$subname."</td><td>".$qns."</td><td>".$ans."</td></tr></tbody>";
        } 
      echo "</table>";
  }
  else{
