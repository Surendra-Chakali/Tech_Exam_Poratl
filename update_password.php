...
<nav class="navbar navbar-expand-md bg-dark navbar-dark">
		  ...
		  <!-- Navbar links -->
			 <div class="collapse navbar-collapse justify-content-end align-items-center" id="navigation">
			    <ul class="navbar-nav d-flex align-items-center">
			      ...
			      <li class="nav-item">
			        <a class="nav-link text-white" href="student_login.php">Login</a>
			      </li>
			    </ul>
			 </div>
		</nav>
    ...
    $newpasserror = $cpasserror = $lengtherror = "";
...
include("db_connection.php");
    ...
    if(!empty($newpass)) 
    {
      if(!empty($cpass))
      {
        ...
        if($newpass == $cpass)
        {
   $sql = mysqli_query($conn, "update students set pwd = '$newpass', cpwd = '$cpass' where rollnum = '$rollnum'");
   if($sql){
    ?>
              <script type="text/javascript">
                alert("Password updated successfully");
              </script>
              <meta http-equiv="refresh" content="0,url=/TecExam/student_login.php">
    <?php

            }else{echo "<p class='text-center text-danger'>Sorry! please try again</p>";}
        }else{echo "<p class='text-center text-danger mt-2'>Conform password do not match</p>";}
        }else{echo "<p class='text-danger text-center mt-2'>At least 8 characters are required</p>";}
      }
        else{$cpasserror = "Enter Conform Password";}
        }
        else{$newpasserror = "Please enter password";}
      }   
  }
    ?>
    ...
    <form method="post" class="mt-4">
        ...
        <p class="text-danger passerror" >Your password must contain at least 8 characters, one Capital letter, one small letter, one special character and one number.</p>
          <p class="text-danger"><?php echo $newpasserror;?></p>          
        </div>
        <div class="form-group">
     ...
     <input type="password" class="form-control text-center" id="cpwd" placeholder="Conform Password" name="cpass">
        ...
        <p class="text-danger pwd_match">Password do not match</p>
          <p class="text-danger"><?php echo $cpasserror;?></p>          
        </div>
